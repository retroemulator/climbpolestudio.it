...
export function WhatsappFab() {
  ...
  return (
    <div
      className={`fixed bottom-5 right-5 z-40 transition-all duration-300 print:hidden ${
        show ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-4 opacity-0"
      }`}
    >
      {/* Sonar: anelli che pulsano per attirare l'attenzione (off con reduced-motion) */}
      <span aria-hidden className="cps-fab-ring absolute inset-0 rounded-full bg-brand-strong" />
      <span
        aria-hidden
        className="cps-fab-ring absolute inset-0 rounded-full bg-brand-strong"
      ...
      <a
        ...
        className="cps-fab relative inline-flex h-14 w-14 items-center justify-center rounded-full bg-brand-strong text-paper transition-transform duration-300 hover:scale-110"
      >
